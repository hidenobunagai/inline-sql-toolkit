...
import { type PythonTokenType, tokenizePython } from "../python-analysis/tokenizer.js";
...
function pythonTypeToSemantic(type: PythonTokenType): SemanticTokenType | undefined {
  switch (type) {
    case "name":
      return "variable";
    case "string":
...
export function createInlineSqlSemanticTokensProvider(): {
  ...
  const build = (
    ...
    const sqlRanges = literals.map(
      (literal) =>
        new vscode.Range(
          offsetToPosition(literal.start, lineStarts),
          offsetToPosition(literal.end, lineStarts),
        ),
    );
    for (const pyToken of tokenizePython(text)) {
      if (isCancellationRequested(token)) return false;
      const semanticType = pythonTypeToSemantic(pyToken.type);
      ...
      builder.push(new vscode.Range(start, end), semanticType, []);
    }
